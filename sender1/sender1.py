...
def create_data_packet(packet_type, seq_num, length, file_name):
    udp_header = struct.pack('!cII', bytes(packet_type, 'utf-8'), socket.htonl(seq_num), length)

    payload = bytes(file_name, 'utf-8') # TODO: retrieve length amonut of bytes from  file_name and set pointer
...
if __name__ == "__main__":
    ...
    # sender and requester port should be in range: 2049 < port < 65536
    ...
    # 4 bytes: length
    udp_header = struct.pack("!cII", bytes('D', 'utf-8'), 0, length)
